...
{\bf Results and Analysis.} 
	...
	% the CC of assumed semi-factives \emph{discover, see} does not have higher projection-ratings under negation than other operators. \textbf{Figure~\ref{fig:figure1}} clearly shows that mean certainty ratings are lower for negation than conditionals and questions.
	%
	...
	%
	\noindent{\bf LH:} Alternatively, we might only talk about the missed predictions for the semi-factives here? \\
	...
	\begin{tabular}{p{.663\linewidth} p{.3\linewidth}}
		\scalebox{.77}{
			\begin{tabular}{llrrrr}
				...
				\midrule
				\#1 & Intercept: \emph{question} & 0.52 & 0.01 & 87.31 & ***\\
				& operator: conditional & 0.04 & 0.01 & 5.07 & ***\\
				& operator: modal & -0.04 & 0.01 & -4.55 & ***\\
				& operator: negation & -0.03 & 0.01 & -4.30 & ***\\
				\midrule
		...
		&
		\parbox{\linewidth}{\caption{\small Excerpt of the output from three linear mixed effects models; \textbf{\#1} has fixed effects of operator; random effect: participant intercepts, \textbf{\#2-4} have fixed effect: operator, predicate, and their interaction; random effect: participant intercepts.
		Models were fit with \texttt{lme4, lmertest} in \texttt{R}. All three models also had at least $34$ highly significant interaction terms of \texttt{operator} and \texttt{predicate} with $p < 0.001$ (Not shown here).}\label{t:models}}
