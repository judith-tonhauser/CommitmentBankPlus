...
To investigate \citepos{mandelkern-etal2020} claim that their naturalness rating measure provides support for a categorical distinction between presuppositions and nonpresuppositions, participants read two-sentence discourses consisting of a declarative (which provided the context) and an interrogative (which realized a (non)factive predicate), and rated the naturalness of the interrogative in the context of the declarative.
...
\hline
\end{tabular}
\caption{Posterior distributions of estimated marginal means of pairwise differences in the explicit ignorance context, with expressions ordered by mean naturalness rating in the same context. Color coding indicates whether the difference between the naturalness ratings for contents of the expressions in the first column and those of the expressions in the top row were positive or negative, and the size of the difference; a white cell means that there was no difference: \\
...
\caption{Mean naturalness rating by predicate (\fcolorbox{black}{orange}{factive}, \fcolorbox{black}{green}{nonfactive}) and context, with predicates ordered as in Fig.~\ref{fig:acc-by-expression}. Error bars indicate 95\% bootstrapped CIs. Violin plots indicate kernel probability density of participants' ratings. At the bottom of each facet, a line spanning two contexts  indicates that the ratings in the two contexts differ according to the pairwise comparison. The linetype indicates  whether the difference is larger than 1.5 (solid line: ---), below 1.5 but above 0.5 (dashed line: -- --), or below 0.5 and above 0 (dotted line: \raisebox{1mm}{\ldots}). }\label{fig:acc-by-context}
\end{figure}
...
The experiment was designed to investigate \citepos{mandelkern-etal2020} claim that the measure used in their Exp.~3 provides support for a categorical distinction between presuppositions and nonpresuppositions. Neither the comparison of naturalness ratings in explicit ignorance contexts  in \S\ref{s:analysis1} nor the comparison of naturalness ratings in explicit ignorance and support contexts in \S\ref{s:analysis2} provided empirical support for a categorical distinction between the CCs of factive and nonfactive predicates, or, more generally, between presuppositions and nonpresuppositions. Rather, the results align with that of \citealt{degen-tonhauser-language}, namely that factive predicates are not a natural class that is categorically different from nonfactive predicates. In this section, we discuss theoretical implications of the results.
