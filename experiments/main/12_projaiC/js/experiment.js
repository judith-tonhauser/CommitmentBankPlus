...
//	  var utterance = "<table><tr><td><strong>"+this.stim.name + ":</strong> \"<i>"+this.stim.utterance+"</i>\"</td></tr><tr><td><strong>"+this.stim.name2 + ":</strong> \"<i>Are you sure?</i>\""+"</td></tr><tr><td><strong>"+this.stim.name + ": </strong> \"<i>Yes, I'm sure that "+this.stim.question+".</i>\""+"</td></tr></table>"
      var utterance = "";
      var continuation = this.stim.continuation;

      console.log("utterance", this.stim.utterance);
      var continuation2 = this.stim.utterance.split(",")[1];
      console.log("continuation2 ", continuation2);

      console.log("this.stim.triggerclass", this.stim.trigger_class)
      if(this.stim.trigger_class!== "MC"){
        continuation2 = continuation2.replace("will be", "won't be")
        continuation2 = " and " + continuation2
        console.log("continuation2",continuation2)
        }
      else{
        continuation2 = "."
      ...
      console.log("this.stim.continuation",this.stim.continuation)

      if (this.stim.block == "ai") {
	  		utterance = "<table><tr><td><strong>"+this.stim.name + ":</strong> \"<i>" +this.stim.utterance+"</i>\"</td></tr><tr><td><strong>"+this.stim.name2 + ":</strong> \"<i>Yes, that's true, but " + this.stim.subject + " " + continuation + continuation2 + "</i>\"";
        // controls: 
...
//    },
   {
     "trigger":"be_annoyed",
     "trigger_class":"C",
     "continuation":"won't be annoyed about it"
   }, 
   {
     "trigger":"discover",
     "trigger_class":"C",
     "continuation":"won't discover it"
   }, 
   {
     "trigger":"know",
     "trigger_class":"C",
     "continuation":"won't know it"
   }, 
   {
     "trigger":"reveal",
     "trigger_class":"C",
      "continuation":"won't reveal it"
   },
   {
     "trigger":"see",
     "trigger_class":"C",
      "continuation":"won't see it"
   },
   {
     "trigger":"pretend",
     "trigger_class":"C",
      "continuation":"won't pretend it"
   }, 
   {
     "trigger":"suggest",
     "trigger_class":"C",
    "continuation":"won't suggest it"
   }, 
   {
     "trigger":"say",
     "trigger_class":"C",
    "continuation":"won't say it"
   }, 
   {
     "trigger":"think",
     "trigger_class":"C",
    "continuation":"won't think it"
   },
   {
     "trigger":"be_right",
     "trigger_class":"C",
    "continuation":"won't be right about it"
   },
   {
     "trigger":"demonstrate",
     "trigger_class":"C",
     "continuation":"won't demonstrate it"
   },
   {
     "trigger":"acknowledge",
     "trigger_class":"C",
     "continuation":"won't acknowledge it"
   },
   {
     "trigger":"admit",
     "trigger_class":"C",
     "continuation":"won't admit it"
   },
   {
     "trigger":"announce",
     "trigger_class":"C",
     "continuation":"won't announce it"
   },
   {
     "trigger":"confess",
     "trigger_class":"C",
     "continuation":"won't confess it"
   },
   {
     "trigger":"confirm",
     "trigger_class":"C",
     "continuation":"won't confirm it"
   },
   {
     "trigger":"establish",
     "trigger_class":"C",
     "continuation":"won't establish it"
   },
   {
     "trigger":"hear",
     "trigger_class":"C",
      "continuation":"won't hear it"
   },
   {
     "trigger":"inform",
     "trigger_class":"C",
     "continuation":"won't inform Sam about it"
   },
   {
     "trigger":"prove",
     "trigger_class":"C",
     "continuation":"won't prove it"
   }
 ]);
 ...
 var contents = {
   "mary": {
     ...
     "pretend_continuation": "won't pretend that she is",
     "think_continuation":"won't think that she is",
   ...
   "josie": {
     ...
     "subject":"Sarah",
      ...
      "pretend_continuation":"won't pretend that she did",
     "think_continuation":"won't think that she did",
   ...
   "olivia": {
    ...
    "pretend_continuation":"won't pretend that she does",
     "think_continuation":"won't think that she does",
  ...
  "be_annoyed":"If Andrea is annoyed that Isabella ate a steak on Sunday, Liz will be delighted.",
   ...
   "danny": {
     ...
     "subject":"Kathryn",
      ...
      "pretend_continuation":"won't pretend that he did",
     "think_continuation":"won't think that he did",
  ...
  "be_annoyed":"If Randy is annoyed that Jackson ran 10 miles, Kayla will be cheerful.",
   ...
   "julian": {
     ...
     "pretend_continuation":"won't pretend that he does",
     "think_continuation":"won't think that he does",
...
var mcitems = {
  "muffins": {
    ...
    "MCcontinuation":"they aren't really delicious",
    "subject":""},
  "pizza": {
    ...
    "MCcontinuation":"you didn't just make it from scratch",
    "subject":""},
  "kids": {
    ...
    "MCcontinuation":"he's not your long-time neighbor",
    "subject":""},
"ballet": {
    ...
    "MCcontinuation":"she isn't a local performer",
    "subject":""},
"garage": {
    ...
    "MCcontinuation":"they're not very well-behaved",
    "subject":""},
"hat": {
    ...
    "MCcontinuation":"she's not really into fashion",
    "subject":""},
};
...
//   	"MC5": getContent("MC")
  ...
  function makeStim(i,prior) {
    ...
    console.log("before item.continuation",item.continuation)


    if (trigger == "pretend"){
      continuation = contents[trigger_cont].pretend_continuation
    }
    else if (trigger == "think"){
      continuation = contents[trigger_cont].think_continuation
    }
    else {continuation = item.continuation
    }
...
//    console.log(contents[trigger_cont]);    
    ...
    var subject = contents[trigger_cont].subject;
...
//    console.log(contents[trigger_cont]);    
    ...
    var question = mcitems[j].question;
    var continuation = mcitems[j].MCcontinuation;   
//    console.log(contents[trigger_cont]); 
    return {
    "name": name,
    "gender": gender, 
    "name2": name2,
    "gender2": gender2,  
    "subject":"",//no named subject for MC controls
    "trigger": trigger,
    "short_trigger": short_trigger,   
    "trigger_class": "MC",
      "continuation": continuation,  
