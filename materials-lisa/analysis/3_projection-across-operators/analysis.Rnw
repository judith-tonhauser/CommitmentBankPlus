...
# setwd(this.dir)
setwd("/Users/lisa/Documents/research/GPP-exps/materials-lisa/analysis/3_projection-across-operators")
...
<<freq-linear-suggest-m, message=FALSE, warning=FALSE>>=
...
levels(data$op)
# contrasts(data$op)
### dummy coding with "m" as baseline
...
# glmm1 <- lmer(projective ~ verb * op + (1 | workerid) +
#                  (1 | content), data=data)
...
<<freq-linear-annoyed-q, message=FALSE>>=

# dummy coding for "verb" with "suggest" as baseline
data$verb <- relevel(data$verb, ref = "be_annoyed")

### dummy coding for "op" with "q" as baseline
data$op <- relevel(data$op, ref = "q")

# glmm2a <- lmer(projective ~ verb * op + (1 | workerid) +
#                  (1 | content), data=data)

# save.image("../../../results/main/meta-analyses/projectivity/rscripts/linear-models.RData")
# load("../../../results/main/meta-analyses/projectivity/rscripts/linear-models.RData")
print(summary(glmm2a), cor=F, dig=3)
...
<<freq-linear-know-m, message=FALSE>>=
...
# glmm4 <- lmer(projective ~ verb * op + (1 | workerid) +
#                  (1 | content), data=data)
#
# save.image("../../../results/main/meta-analyses/projectivity/rscripts/linear-models.RData")
# load("../../../results/main/meta-analyses/projectivity/rscripts/linear-models.RData")
print(summary(glmm4), cor=F, dig=3)

@

\subsection{Linear frequentist with \texttt{annoyed/n} as baseline}
<<freq-linear-annoyed-n, message=FALSE>>=
...
# glmm6 <- lmer(projective ~ op * verb + (1 | workerid) +
#                  (1 | content), data=data)

# save.image("../../../results/main/meta-analyses/projectivity/rscripts/linear-models.RData")
# load("../../../results/main/meta-analyses/projectivity/rscripts/linear-models.RData")
print(summary(glmm6), cor=F, dig=3)
