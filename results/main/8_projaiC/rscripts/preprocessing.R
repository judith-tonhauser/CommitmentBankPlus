...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#212 female, 30 male, 8 other, 0 undeclared
...
d <- d %>%
  filter(american == "Yes") %>%
  droplevels()
length(unique(d$workerid)) #250 (data from 0 participants excluded)
...
d.MC <- d %>%
  ...
  droplevels()
nrow(d.MC) #3000 / 250 participants = 12 (6 main clause controls in each of the two blocks)
...
nrow(outliers) #312 / 12 = 26 outlier participants
