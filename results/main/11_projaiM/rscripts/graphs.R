...
ggplot(toplot, aes(x=AIMean,y=ProjMean,group=1)) +
  ...
  geom_point() +
  xlab("Mean not-at-issueness rating ('yes, that's true, but not c' ')") +
  ylab("Mean certainty ratings") +
  xlim(0,1) +
  ylim(0,1) 
ggsave("../graphs/proj-by-ai-polar-Q.pdf",width=5,height=3.5)
...
cor(toplot$AIMean,toplot$ProjMean) #-.19
