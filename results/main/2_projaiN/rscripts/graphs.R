...
ggplot(toplot, aes(x=AIMean,y=ProjMean,group=1)) +
  ...
  xlim(0,1) +
  ylim(0,1) 
ggsave("../graphs/proj-by-ai-negated.pdf",width=4.2,height=3.5)
...
# correlation between at-issueness and projectivity by predicate and content:
means = t %>%
  filter(short_trigger != "MC") %>%
  droplevels() %>%
  group_by(short_trigger, content) %>%
  summarize(AIMean = mean(ai), ProjMean = mean(projective))
cor(means$AIMean,means$ProjMean) #.16
...
ggplot(t, aes(x=ai,y=projective)) +
  ...
  theme(legend.position="top")
ggsave("../graphs/by-participant-correlation-negation.pdf",width=20,height=25)
