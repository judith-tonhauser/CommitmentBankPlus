...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#182 female, 64 male, 4 other, 0 undeclared
...
d <- d %>%
  filter(language != "Spanish" & language != "Male" & language != "Arabic") %>%
  droplevels()
length(unique(d$workerid)) #247 (data from 3 participant excluded)
...
d <- d %>%
  filter(american == "Yes") %>%
  droplevels()
length(unique(d$workerid)) #247 (data from 0 participants excluded)
...
round(mean(d.MC.AI$response),2) #.53
