...
ggplot(toplot, aes(x=AIMean,y=ProjMean,group=1)) +
  ...
  geom_point() +
  xlab("Mean not-at-issueness rating ('sure that')") +
  ylab("Mean certainty ratings") +
  xlim(0,1) +
  ylim(0,1) 
ggsave("../graphs/proj-by-ai-modal.pdf",width=4.2,height=3.5)
...
cor(toplot$AIMean,toplot$ProjMean) #-.62
...
cor(means$AIMean,means$ProjMean) #-.2
...
ggplot(t, aes(x=ai,y=projective)) +
  ...
  theme(legend.position="top")
ggsave("../graphs/by-participant-correlation-modal.pdf",width=20,height=25)

## check if block order mattered here -----
...
ai.means2 = proj_block1 %>%
  group_by(short_trigger) %>%
  summarize(AIMean = mean(ai))
ai.means2

# mean projectivity by predicate, including the main clause controls
proj.means1 = ai_block1 %>%
  group_by(short_trigger) %>%
  summarize(ProjMean = mean(projective))
proj.means1

proj.means2 = proj_block1 %>%
  group_by(short_trigger) %>%
  summarize(ProjMean = mean(projective))
proj.means2
...
tmp1 = ai.means1 %>%
  left_join(proj.means1, by=c("short_trigger")) %>%
  filter(short_trigger != "MC") %>%
  droplevels()


tmp2 = ai.means2 %>%
  left_join(proj.means2, by=c("short_trigger"))  %>%
  filter(short_trigger != "MC") %>%
  droplevels()


# correlation between at-issueness and projectivity by predicate:
# without block differentiation: -.62
cor(tmp1$AIMean,tmp1$ProjMean) # -.6
cor(tmp2$AIMean,tmp2$ProjMean) # -.5
