...
# graphs
# testing for correlation of positive (!) values on both diagnostics
...
ggplot(ai.means, aes(x=verb, y=Mean, fill=VeridicalityGroup)) +
  ...
  xlab("Predicate (under negation)") +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1)) 
ggsave("../graphs/ai-by-predicate.pdf",height=4,width=7)
...
ggplot(toplot, aes(x=AIMean,y=ProjMean,fill=VeridicalityGroup)) +
  ...
  guides(fill=FALSE) +
  xlab("Mean at-issueness rating \n (assent)") +
  ylab("Mean certainty ratings") +
  xlim(0,1) +
  ylim(0,1) 
ggsave("../graphs/proj-by-+ai.pdf",width=4.2,height=3.5)
...
cor(toplot$AIMean,toplot$ProjMean) #-.69
...
cor(means$AIMean,means$ProjMean) #-.46

# by-participant projection by not-at-issueness
ggplot(cd, aes(x=ai,y=projective)) +
  geom_smooth(method="lm") +
  geom_point() +
  xlab("At-issueness rating (assent)") +
  ylab("Certainty rating") +
  xlim(0,1) +
  ylim(0,1) +
  facet_wrap(~short_trigger) +
  theme(legend.position="top")
ggsave("../graphs/by-participant-correlation-+ai-neg.pdf",width=20,height=25)
