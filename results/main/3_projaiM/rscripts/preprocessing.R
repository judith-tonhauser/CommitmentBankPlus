# modal: projection and at-issueness
# preprocessing
...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#150 female, 141 male, 7 other, 2 undeclared
...
table(ds$language) 
# nobody doesn't list english

#d <- d %>%
#  filter(language != "Chinese" & language != "Russian" & language != "telugu") %>%  droplevels()
#length(unique(d$workerid)) # (data from 0 Turker excluded, 10 remaining Turkers)
...
d.MC <- d %>%
  ...
  droplevels()
nrow(d.MC) #3600 (300 Turkers x 6 MCs x 2 questions)
...
round(mean(d.MC.Proj$response),2) #.96 (because speaker is committed to content of positive indicative sentences)
...
round(mean(d.MC.AI$response),2) #.03 (because main clause content is at-issue, coded as 0)
...
length(unique(outliers$workerid)) #19 outlier Turkers

# exclude all outliers identified above
d <- d %>%
  filter(!(workerid %in% p$workerid | workerid %in% ai$workerid)) %>%
  droplevels()
length(unique(d$workerid)) #281 remaining Turkers (19 Turkers excluded)
...
#length(unique(d$workerid)) #9 Turkers remain

# age and gender of remaining participants
table(d$age) #18-74
length(which(is.na(d$age))) #1 missing value
...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#144 female, 129 male, 7 other, 1 undeclared
