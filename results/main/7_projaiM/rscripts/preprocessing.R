...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#132 female, 113 male, 6 other, 0 undeclared
...
nrow(outliers) #324 / 12 = 27 outlier participants

# exclude all outlier participants identified above
d <- d %>%
  filter(!(workerid %in% p$workerid | workerid %in% ai$workerid)) %>%
  droplevels()
length(unique(d$workerid)) # 220 remaining participants (27 participants excluded)
...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#116 female, 98 male, 6 other, 0 undeclared
