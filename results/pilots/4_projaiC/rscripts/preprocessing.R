...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#8 female, 2 male, 0 other, 0 undeclared
...
round(mean(d.MC.Proj$response),2) #.97 (because speaker is committed to content of positive indicative sentences)
...
round(mean(d.MC.AI$response),2) #.11 (because main clause content is at-issue, coded as 0)
...
nrow(outliers) #24 (2 Turkers x 12 = 6 main clauses x 2 questions)
...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#6 female, 2 male, 0 other, 0 undeclared
