# 11_projaiM
# modal: projection and at-issueness (assent with adversative continuation)
# preprocessing
...
ds = read_csv("../data/experiment-subject-information.csv")
#View(ds)
...
round(mean(d.MC.Proj$response),2) #.98 (because speaker is not committed to content of questions)
...
round(mean(d.MC.AI$response),2) #.36 (because main clause content is at-issue, coded as 0)
...
d <- d %>%
  filter(!(workerid %in% p$workerid | workerid %in% ai$workerid)) %>%
  droplevels()
length(unique(d$workerid)) # 8 remaining Turkers (1 Turkers excluded)
