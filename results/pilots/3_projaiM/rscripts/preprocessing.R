...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#3 female, 5 male, 2 other, 0 undeclared
...
d.MC <- d %>%
  ...
  droplevels()
nrow(d.MC) #120 (10 Turkers x 6 MCs x 2 questions)
...
round(mean(d.MC.Proj$response),2) #.98 (because speaker is committed to content of positive indicative sentences)
...
round(mean(d.MC.AI$response),2) #.01 (because main clause content is at-issue, coded as 0)
...
nrow(outliers) #12 (1 unique outlier Turkers x 12 = 6 main clauses x 2 questions)

# exclude all outliers identified above
d <- d %>%
  filter(!(workerid %in% p$workerid | workerid %in% ai$workerid)) %>%
  droplevels()
length(unique(d$workerid)) # 9 remaining Turkers (1 Turkers excluded)
...
d %>% 
  ...
  group_by(gender) %>% 
  summarize(count=n())
#3 female, 4 male, 2 other, 0 undeclared
